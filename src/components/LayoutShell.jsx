...
function getPageMeta(pathname) {
  ...
  if (pathname.startsWith("/obra/")) return { title: "Obra pública", backTo: "/discover" };
...
export default function LayoutShell() {
  ...
  return (
    <div className="flex h-screen overflow-hidden bg-background">
      ...
      >
        <div className="flex items-center justify-between border-b border-sidebar-border px-5 py-5" style={{ paddingTop: "max(1.25rem, var(--safe-top))" }}>
          <Link to="/" className="flex items-center gap-2.5">
            ...
            <span className="font-semibold tracking-tight text-sidebar-foreground">Escritório</span>
          </Link>
          ...
          <div className="border-t border-sidebar-border p-3">
            <Link to="/settings" className="flex items-center gap-3 rounded-lg px-2 py-2.5 transition-colors hover:bg-sidebar-accent">
              ...
              <div className="min-w-0 flex-1">
                <p className="truncate text-sm font-medium leading-tight text-sidebar-foreground">{user.display_name || user.full_name || "Usuário"}</p>
        ...
        <div className="sticky top-0 z-30 border-b border-border bg-background/95 backdrop-blur lg:hidden" style={{ paddingTop: "var(--safe-top)" }}>
          <div className="flex items-center gap-3 px-4 py-3">
            ...
            <div className="flex min-w-0 items-center gap-2">
              ...
              <span className="truncate text-sm font-semibold">{pageMeta?.title || "Escritório"}</span>
            </div>
          </div>
        </div>
      ...
      <CreatePublicWorkDialog open={showCreatePublicWork} onOpenChange={setShowCreatePublicWork} onSuccess={() => navigate("/discover")} />
    </div>
  );
}
